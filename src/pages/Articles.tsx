...
import { getAllArticles, formatArticleDate, ArticleMeta } from '../lib/articles'
import ReactMarkdown from 'react-markdown'
import remarkGfm from 'remark-gfm'
import { BookOpen, Clock, ArrowLeft, ArrowRight, User, Calendar, Tag } from 'lucide-react'

export default function Articles({ navigate }: { navigate: (p: Page) => void }) {
  const { lang } = useLang()
  ...
  const tagsEn = ['All', ...Array.from(new Set(articles.map(a => a.tag.en)))]
  const tagsId = ['Semua', ...Array.from(new Set(articles.map(a => a.tag.id)))]
  const tags = lang === 'en' ? tagsEn : tagsId

  const selectedTagEn = tagsEn[activeIdx] || 'All'

  const filteredArticles = activeIdx === 0
    ? articles
    : articles.filter(a => a.tag.en === selectedTagEn || a.tag.id === tagsId[activeIdx])

  const selectedArticle = articles.find(a => a.slug === selectedSlug)

  if (selectedArticle) {
    const title = lang === 'en' ? selectedArticle.title.en : selectedArticle.title.id
    const tag = lang === 'en' ? selectedArticle.tag.en : selectedArticle.tag.id
    const summary = lang === 'en' ? selectedArticle.summary.en : selectedArticle.summary.id
    const content = lang === 'en' ? selectedArticle.contentEn : selectedArticle.contentId
      ...
      <main className="w-full bg-[#F8FAFC] py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <span>{lang === 'en' ? 'Back to all articles' : 'Kembali ke semua artikel'}</span>
          </button>
          ...
          <article className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-12">
            <div className="p-6 sm:p-10 border-b border-slate-100">
              <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500 mb-4">
                ...
                <span className="inline-flex items-center gap-1">
                  ...
                  {selectedArticle.readTime} {lang === 'en' ? 'min read' : 'menit baca'}
                </span>
                <span>•</span>
            ...
            <div className="p-6 sm:p-12 prose prose-slate max-w-none prose-headings:font-['Barlow_Condensed'] prose-headings:font-bold prose-h1:text-3xl prose-h2:text-2xl prose-h2:text-[#0F172A] prose-h2:border-b prose-h2:border-slate-200 prose-h2:pb-2 prose-h3:text-xl prose-p:text-slate-700 prose-p:leading-relaxed prose-a:text-[#0284C7] prose-code:text-[#0284C7] prose-code:bg-slate-100 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-pre:bg-slate-900 prose-pre:text-slate-100 prose-table:w-full prose-td:p-3 prose-th:p-3 prose-th:bg-slate-100">
          ...
          <div className="bg-[#0F172A] text-white p-8 sm:p-10 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
              <h3 className="font-['Barlow_Condensed'] font-bold text-2xl sm:text-3xl mb-2">
                {lang === 'en' ? 'Need embedded systems support?' : 'Butuh dukungan sistem tertanam?'}
              </h3>
            ...
            >
              <span>{lang === 'en' ? 'Talk to an Engineer' : 'Hubungi Insinyur'}</span>
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between lg:items-end gap-8">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#0284C7]/10 text-[#0284C7] font-semibold text-xs tracking-wider uppercase mb-3">
              ...
              <span>{lang === 'en' ? 'Engineering Articles' : 'Artikel Rekayasa'}</span>
            </div>
            <h1 className="font-['Barlow_Condensed'] font-bold text-4xl sm:text-6xl text-[#0F172A] leading-none">
              {lang === 'en' ? 'Technical writing without the fluff.' : 'Tulisan teknis tanpa basa-basi.'}
            </h1>
          </div>
                ...
                <div>
                  <div className="flex items-center gap-3 text-xs font-semibold text-slate-500 mb-4 flex-wrap">
                    <span className="px-2.5 py-1 rounded-md bg-[#0284C7]/10 text-[#0284C7] font-bold uppercase tracking-wider">
                      {lang === 'en' ? featured.tag.en : featured.tag.id}
                    </span>
                    <span>•</span>
                    ...
                    <span className="flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5 text-slate-400" />
                      {featured.readTime} {lang === 'en' ? 'min read' : 'menit baca'}
                    </span>
                  </div>
                  ...
                  <h2 className="font-['Barlow_Condensed'] font-bold text-2xl sm:text-4xl text-[#0F172A] mb-4 leading-snug group-hover:text-[#0284C7] transition-colors">
                    {lang === 'en' ? featured.title.en : featured.title.id}
                  </h2>

                  <p className="text-slate-600 text-base leading-relaxed mb-8">
                    {lang === 'en' ? featured.summary.en : featured.summary.id}
                  </p>
                </div>

                <div className="inline-flex items-center gap-2 text-[#0284C7] font-semibold text-sm group-hover:text-[#0369A1] transition-colors">
                  <span>{lang === 'en' ? 'Read full article' : 'Baca artikel lengkap'}</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {rest.map(article => (
                <ArticleCard
                  key={article.id}
                  article={article}
                  onSelect={() => setSelectedSlug(article.slug)}
                />
              ))}
            </div>
          )}
...
function ArticleCard({ article, onSelect }: { article: ArticleMeta; onSelect: () => void }) {
  const { lang } = useLang()
        ...
        <div className="p-6">
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-3 flex-wrap">
            ...
            <span className="flex items-center gap-1">
              ...
              {article.readTime} {lang === 'en' ? 'min read' : 'menit baca'}
            </span>
          </div>
